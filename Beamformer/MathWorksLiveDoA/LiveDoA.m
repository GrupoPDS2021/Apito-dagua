...
% ACHTUNG: não funciona para Matlab 2014 !!!
...
% are driven by microphones arranged on a linear array. If no microphone 
% array or multichanel audio card is available, then set sourceChoice to 
% 'recorded'. In this mode the example will use prerecorded audio samples, 
% acquired with a linear array. For sourceChoice = 'live', the following 
% code uses audioDeviceReader to acquire 4 live audio channels through a 
% Microsoft Kinect™ for Windows®. To use another microphone array setup, 
% ensure the installed audio device driver is one of the conventional types 
% supported by MATLAB and set the Device property of audioDeviceReader 
% accordingly. You can query valid Device assignments for your computer 
% by calling getAudioDevices method of audioDeviceReader object. Note that 
% even when using Microsoft Kinect, the device name can vary across 
% machines and may not match the one used in this example. Use tab 
% completion to get the correct name on your machine.
%-------------------------------------------------------------------------
...
%-------------------------------------------------------------------------
% The following values identify the approximate linear coordinates of the 
% 4 built-in microphones of the Microsoft Kinect™ relative to the position 
% of the RGB camera (not used in this example). For 3D coordinates use 
...
% selected. The larger the inter-microphone distance the more sensitive 
% the DOA estimate. Each column of the following matrix decribes a choice 
% of microphone pair within the array. All values must be integers between 
% 1 and length(micPositions).
%-------------------------------------------------------------------------
...
% The main algorithmic building block of this example is a cross-correlator.
